...
export function generateMockHistory(
  ...
  for (let i = count; i >= 0; i--) {
    ...
    readings.push({
      ...
      ph:           parseFloat(clamp(base.ph + phDrift + seededNoise(i, 0.08), 0, 14).toFixed(2)),
      tds:          parseFloat(clamp(base.tds + tdsDrift + seededNoise(i * 2, 8), 0, 2000).toFixed(1)),
      turbidity:    parseFloat(clamp(base.turbidity + turbidityDrift + seededNoise(i * 3, 6), 0, 3000).toFixed(1)),
      flow_lpm:     parseFloat(clamp(base.flow_lpm + flowDrift + seededNoise(i * 4, 0.15), 0, 30).toFixed(2)),
